...
public class PasajerosTests {
	...
	public void testBuscarPasajeros_ok() {
		...
		checker.ok(4, TestPasajeroNoUsar.of("522.322-2", "Maria", "551115-055", Sistema.Categoria.C),
				"La cantidad de operaciones no coincide, 3.222.. es la raiz, a la izq esta 222.4222.., a su derecha 1.211, a su izquierda 522... son 4",
				"Los dattos de la persona no coinciden",
				sistema.buscarPasajero("522.322-2"));

		checker.ok(4, TestPasajeroNoUsar.of("7.222.522-2", "roberto", "03031452", Sistema.Categoria.D),
				"La cantidad de operaciones no coincide, 3.222.. es la raiz, a la der esta 5.222.., a su derecha 6.222, a su der 7.222... son 4",
				"Los dattos de la persona no coinciden",
	...
	/**
	 * Tratamos de registrar pasajeros invalidos y buscarlos, buscamos tambien
	 * ppasajeros que no estan registrados
	 */
	@Test
	public void testBuscarPasajeros_error() {
		...
		checker.ok(3, TestPasajeroNoUsar.of("522.322-2", "Maria", "551115-055", Sistema.Categoria.C),
				"La cantidad de operaciones no coincide, 3.222.. es la raiz, a la izq esta 222.4222.., a su derecha 522... son 3",
				"Los dattos de la persona no coinciden",
				sistema.buscarPasajero("522.322-2"));
	...
	public void testBuscarPasajerosOrdenados_ok() {
		checker.ok(0, new TestPasajeroNoUsar[] {}, "la cantidad debe ser la cantidad que hay hasta ahora: 0",
				"Tiene que ser vacio",
		...
		TestPasajeroNoUsar[] pasajerosOrdAscendente = new TestPasajeroNoUsar[] {
				...
				// TestPasajeroNoUsar.of("4.212.322-2", "Maria Eugenia", null,
				// Sistema.Categoria.C),
		...
		checker.ok(0, pasajerosOrdAscendente,
				"La cantidad es la cantidad de los que registramos exitosamente",
				"El orden esta mal, chequear ord lexicografico vs numerico",
				sistema.listarPasajerosAscendente());
	...
	/**
	 * Este test busca agregar cosas aleatorias esperando que la implementacion este
	 * correcta,
	 * idealmente loo van a querer correr cuando este terminado el modulo de
	 * pasajeros.
	...
	public void prubasAleatorias() {
		...
		rand.setSeed(semilla);// ESTO ES POR SI LO QUIEREN REPRODUCIR, poner un print, cambian el seed por el
													// valor de la consola
		...
		for (int i = 0; i < CANTIDAD_A_GENERAR; i++) {
			cedulaGeneradaActual += rand.nextInt((CEDULA_MAX - CEDULA_INICIAL) / CANTIDAD_A_GENERAR) + 1;// Basicamente este
																																																		// bound lo ponemos
																																																		// para noo generar
																																																		// cedulas invalidas
			cedulas[i] = cedulaGeneradaActual;
			pasajerosOrdenados[i] = TestPasajeroNoUsar.of(formatearCedula(cedulaGeneradaActual), "Unnombre", "2232",
					Sistema.Categoria.C);
		}
		...
		Logger.getAnonymousLogger()
				.info(String.format("La cantidad promedio de accesos es %.2f contra el de una lista sería %.2f",
						promedioDeAccesos,
		...
		// 5 es un numero magico pero no se deberia ir mas de eso mientras se mantengan
		// en una cantidad esperada.
		// Si ven que es 10 la diff todo bien si las cantidades son muy grandes, ahora
		// no se puede ir mucho mas de 100
	...
	private double cantidadEsperadaAccesos(int n) {
		...
		// sino creen es porque Hn~~ 0.57... + log(n) cuando n es grande, log es
		// neperiano
		// entonces queda ((0.5+log(n)*(n+1)-4*n)/n
		// simplificando queda (0.5+log(n))-4 +(0.5+log(n))/n que es cercano a
		// 0.5+log(n)-4 que en O queda O(log(n))
